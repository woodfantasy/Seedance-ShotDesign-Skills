...
| 기능 | 설명 |
|------|------|
| 🎭 **연기급 미세 표정 시스템** | 얼굴 7개 영역 부위별 어휘집(눈/눈썹/입술/코/턱목/이마/볼) + L1-L4 4단계 표정 강도 체계 + 2초 정밀도 감정 아크 연출(4가지 패턴) + 12가지 감정 배우급 분해 + 반(Anti) AI 가짜 얼굴 선언 + 미세 표정 카메라 연동 |
...
└── references/
    ...
    ├── micro-expressions.md     # 연기급 미세 표정 시스템 지식 베이스 (7대 부위/4단계 강도/2초 감정 아크)
    ├── seedance-specs.md        # Seedance 2.0 공식 플랫폼 사양
    ├── quality-anchors.md       # 품질 앵커 & 라이팅 라이브러리 (NPR 소재/라이팅/충돌 매트릭스 포함)
    ├── scenarios.md             # 수직 시나리오 템플릿 (20개 시나리오 + 애니메이션 변형 + 영상 편집 + 물리 댐핑 툴킷)
