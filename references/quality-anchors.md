...
> **以下泛化词通常应替换为物理描述：** `4K` / `8K` / `masterpiece` / `best quality` / `ultra HD` / `超清晰` / `杰作` / `极致画质`。若 4K 只是描述输入参考文件规格，不把它误判为输出分辨率或硬错误。
>
> **为什么要谨慎？** 这些词缺少可执行的光线、材质与运动信息，容易挤占提示词注意力，并不能稳定保证真实质感。
>
> **正确做法：** 用下方的 **物理介质型号** + **光学瑕疵** + **有机质感** 替代。真实感来源于不完美（Organic Imperfections）。

### 反 AI 假脸指导

> **AI 视频最大的破绽不是画质，而是"表情不对"。** 塑料感的皮肤可以用材质词修复，但"假脸"只能靠正确的表情描写方法论来根治。以下 AI 表情陷阱严禁出现：
> - **完美对称的面部表情**（真人面部永远不完全对称——用 `左侧嘴角微微上扬` 替代 `嘴角上扬`）
> - **情绪瞬间切换、没有过渡**（真人需要 0.3-1.5 秒的肌肉过渡时间——至少留 1-2 秒过渡）
> - **皮克斯式夸张表情**（嘴巴大张、眉毛飞天、眼睛圆瞪——除非是动画风格，写实中严禁）
> - **面瘫式静态面部**（持续数秒完全不动——真人始终有微小的呼吸起伏、眨眼、肌肉波动）
>
> **正确做法：** 用 [micro-expressions.md](micro-expressions.md) 中的面部分区词库替代笼统的情绪形容词。真实的表演在于"微小的不完美"和"渐进的变化"。默认采用 L2（内敛/可读）强度级别。
