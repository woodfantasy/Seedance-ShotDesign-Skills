...
> Seedance 2.5 支持原生音效、音频引用、音色与多语言对白控制。将对白、环境音、动作音效与 BGM 分层描述，避免混淆语义。
...
- `@音频1` ~ `@音频10`：最多引用 10 段音频，也支持仅上传音频生成视频
- 单段及音频总时长均 ≤ 30 秒；单文件大小 ≤ 15MB
- 支持格式：mp3 / wav
...
> Seedance 2.5 支持通过参考视频或音频提取音色特征，并可直接指定方言和语言。以下能力融入提示词的「原生音效要求」维度。
