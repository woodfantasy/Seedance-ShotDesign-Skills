...
>
> **本地化规则：** 模板以中文编写。最终输出使用用户请求的自然工作语言，不把非中文用户统一转换为英文。英文输出可使用英文时间戳与标签，并从双语词库中选取完整短语；其他语言保留同一提示词结构和准确对白。

---

## 目录

- 电商、仙侠、短剧、美食、MV、一镜到底、微距、汽车、自然风光
- 30–180 秒长视频与游戏 PV
- 恐怖、旅行、宠物、变身、Loop
- 视频编辑、视频延长、分镜图、多帧故事
- 时间静止、多镜头混剪、极限第一人称
...
**核心策略：** 无既有成片时，31–180 秒连续叙事优先使用「超长视频」；恰好 30 秒且需要精确短片控制时可用标准生成；只有在原视频本身需要保留时才使用「视频延长」。完整结构见 `long-form-storytelling.md`。

**超长视频骨架：**
```
全片时长[XX秒]，画幅[比例]，[类型与基调]。
【角色/场景/道具 Bible】[跨段不变量]
【故事概述】[开端→发展→转折→高潮→结尾]
[00:00-00:XX] [建立与状态变化]
...
[最后区间] [回收目标、关系、道具或声音母题，形成明确结尾]
```
...
拆段时仍要定义交接状态：人物位置与朝向、动作动量、道具所有权、镜头方向、光线和音频尾部，避免下一段状态重置。
...
**核心参数：**
- 时长：与原视频一致；为提升编辑稳定性，建议源视频不超过 20 秒
- 必须素材：@视频1（原视频）
- 可选素材：@图片N（替换角色/场景）
...
> Seedance 2.5 能根据少量素材补全剧情逻辑，并可理解分镜脚本图的景别和镜头语言。先明确格子顺序、人物映射和各格时间范围。
...
> 上传关键帧图片或多宫格分镜图，让引擎按声明的阅读顺序补全镜头之间的动作与过渡。这里描述的是 Seedance 2.5 界面能力，不依赖未在使用手册中确认的 CLI 命令。
...
| 30–180秒连续剧情需要精确分镜控制 | **超长视频 + 时间轴** | 统一 Bible，避免角色与世界状态重置 |
| 漫画分格/分镜脚本图 | **多帧故事** | 引擎能读懂分格顺序 |
| 需要精确时间轴控制（卡点/节奏） | **时间戳分镜** | 时间戳与声音节拍更精准 |

### 核心参数

- **素材上限**：平台最多接受 30 张图片；按叙事需要使用，不要求凑满
- **画幅**：随图片比例 或 指定
...
## 二十一、时间静止 / 子弹时间 (Freeze Time)（v1.8.5 新增）
...
## 二十二、多镜头混剪序列 (Multishot Video)（v1.8.5 新增）
...
## 二十三、极限第一人称视角 (Extreme POV / FPV)（v1.8.6 新增）
